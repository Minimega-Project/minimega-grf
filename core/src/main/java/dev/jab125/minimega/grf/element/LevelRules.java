...
public final class LevelRules extends Element {
	public final int ruleType;

	public LevelRules(String ruleType, List<Element> children) {
		this(ruleType.isEmpty() ? 0 : Integer.parseInt(ruleType), children);
	}

	public LevelRules(int ruleType, List<Element> children) {
		super(children);
	...
	static class Type implements ElementType<LevelRules> {
		@Override
		public LevelRules parseSelf(org.w3c.dom.Element element, List<Element> children) {
			return new LevelRules(element.getAttribute("ruleType"), children);
		}
		...
		public org.w3c.dom.Element serializeSelf(
			...
			element.setAttribute("ruleType", Integer.toString(self.ruleType));
