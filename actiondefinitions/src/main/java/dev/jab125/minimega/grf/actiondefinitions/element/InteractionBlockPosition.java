...
public final class InteractionBlockPosition extends ACElement implements ITrigger {

	public final int x, y, z;

	public InteractionBlockPosition(int x, int y, int z, List<Element> children) {
		super(children);
		this.x = x;
		this.y = y;
		this.z = z;
	...
	@Override
	public String getId() {
		return "InteractionBlockPosition";
	}

	public static class Type implements ACType<InteractionBlockPosition> {
		@Override
		public InteractionBlockPosition parseSelf(org.w3c.dom.Element element, List<Element> children) {
			String[] split = unindent(element.getTextContent()).split(",");
			int x = Integer.parseInt(unindent(split[0]));
			int y = Integer.parseInt(unindent(split[1]));
			int z = Integer.parseInt(unindent(split[2]));
			return new InteractionBlockPosition(x, y, z, children);
		}

		@Override
		public org.w3c.dom.Element serializeSelf(
				Document document,
				InteractionBlockPosition self,
				List<org.w3c.dom.Element> children
		) {
			org.w3c.dom.Element element = document.createElement(self.getId());
			element.setTextContent(self.x + "," + self.y + "," + self.z);
